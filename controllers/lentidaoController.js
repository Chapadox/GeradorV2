...
async function perguntas() {
      ...
      const perguntasPadrao = [ 
          ...
          {message: 'SITES/APLICATIVOS APRESENTANDO LENTIDÃO: '},
        ]  

      addAoRelato('CLIENTE INFORMA ESTAR COM LENTIDÃO \n');
...
const acessoAoEquipaPerg = {message: 'FOI POSSIVEL ACESSO NO EQUIPAMENTO: ', choices: [{value: 'sim'}, {value: 'não'}]}
const pergNormalizou = {message: 'FEITO AS ALTERAÇÕES HOUVE NORMALIZAÇÃO ? ', choices:[{value: 'sim'}, {value: 'não'}]};
//

async function newPerg() {
    const acessoAoequipametoPergs = await select(acessoAoEquipaPerg);
        if (acessoAoequipametoPergs == 'sim') { 
